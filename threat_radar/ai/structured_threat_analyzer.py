...
from threat_radar.ai.attack_scenario_generator import AttackScenarioGenerator, AttackScenario
...
class StructuredThreatAnalyzer:
    ...
    def analyze_threat_model(
        ...
        graph_client, attack_paths = self._load_graph_and_find_paths(
            graph_file,
            max_scenarios * 2  # Get more paths than needed for filtering
        )

        if not attack_paths:
            logger.warning("No attack paths found in graph")
            return self._create_empty_report(
                Path(graph_file).stem,
                "No viable attack paths discovered"
            )

        logger.info(f"Discovered {len(attack_paths)} attack paths")

        # Step 2: Get target node data for preference scoring
        target_nodes_data = self._extract_target_nodes_data(
            graph_client,
            attack_paths
        )
        ...
        logger.info(f"Analyzing with {len(personas)} threat actor(s): {[p.name for p in personas]}")
        ...
        high_priority_scenarios = [
            s for s in all_scenarios
            if s not in critical_scenarios
    ...
    def _generate_scenarios_for_actor(
        ...
        for path in ranked_paths[:max_scenarios]:
            try:
                ...
                logger.error(f"Failed to generate scenario for path {path.path_id}: {e}")
                continue
    ...
    def _generate_executive_summary(
            ...
            "scenario_summaries": [
                {
                    ...
                    "narrative_excerpt": s.narrative[:200] + "..." if len(s.narrative) > 200 else s.narrative,
        ...
        try:
            ...
            return self._generate_fallback_summary(scenarios, personas, environment_name)
